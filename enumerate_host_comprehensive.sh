...
find /host -name ".env" -type f 2>/dev/null | head -10 || echo "  No .env files found"
echo ""
...
echo "[*] User crontabs:"
chroot /host ls -la /var/spool/cron/crontabs/ 2>/dev/null || echo "  Cannot read user crontabs"
echo ""

echo "[*] Systemd timers:"
chroot /host systemctl list-timers 2>/dev/null | head -10 || echo "  Cannot read timers"
echo ""
...
echo "[*] SUID binaries:"
find /host -perm -4000 -type f 2>/dev/null | head -20 || echo "  Cannot find SUID binaries"
echo ""

echo "[*] Writable directories:"
find /host/tmp -writable -type d 2>/dev/null 2>/dev/null | head -5 || echo "  /tmp not writable"
find /host/var/tmp -writable -type d 2>/dev/null 2>/dev/null | head -5 || echo "  /var/tmp not writable"
echo ""
...
echo "[*] GCP metadata:"
curl -s --max-time 2 -H "Metadata-Flavor: Google" http://169.254.169.254/computeMetadata/v1/ 2>/dev/null | head -5 || echo "  Not GCP or metadata not accessible"
echo ""
